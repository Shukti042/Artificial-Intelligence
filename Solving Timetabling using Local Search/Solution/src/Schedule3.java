...
public class Schedule3 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<Course> saturPriority=new ArrayList<Course>();
        for (int i = 0; i < courseCount; i++){
            saturPriority.add(courses.get(i));
        }
        int Color=1;
        for (int i=0;i<courseCount;i++)
        {
            boolean[] colorAvaibility=new boolean[Color];
            saturPriority.sort(Course::compareSaturation);
            Course c=saturPriority.remove(0);
        ...
        //Skeaky
        ArrayList<Course> newSlotAvailbility=new ArrayList<Course>();
        for (int i=0;i<students.size();i++)
        {
            boolean flag=false;
            for (int j=0;j<students.get(i).enrolledCourses.size();j++)
            {
                for (int k=j+1;k<students.get(i).enrolledCourses.size();k++)
                {
                    if(Math.abs(students.get(i).enrolledCourses.get(j).color-students.get(i).enrolledCourses.get(k).color)==1)
                    {
                        for(int l=0;l<newSlotAvailbility.size();l++)
                        {
                            if(newSlotAvailbility.get(l).Neighbours.contains(students.get(i).enrolledCourses.get(j)))
                            {
                                flag=true;
                                break;
                            }
                        ...
                        if(!flag) {
                            students.get(i).enrolledCourses.get(j).color = Color;
                            newSlotAvailbility.add(students.get(i).enrolledCourses.get(j));
                            flag=true;
                            break;
                        }
