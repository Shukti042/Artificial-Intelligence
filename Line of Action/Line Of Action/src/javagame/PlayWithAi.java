...
public class PlayWithAi extends BasicGameState {
    ...
    int turn=1;
    Checker[] checkers=new Checker[24];
    float[][] pieceSquareTable=new float[boardsize][boardsize];

    public PlayWithAi(int state) {
    }
    ...
    public void init(GameContainer gc,StateBasedGame sbg) throws SlickException
    {
        ...
        for (int i=6;i<12;i++)
        {
            ...
            checkers[i].color=Color.white;
            checkers[i].colorCode=0;
            checkers[i].cell=board[boardsize-1][i-5];
            board[boardsize-1][i-5].checker=checkers[i];
            checkers[i].alive=true;
            checkers[i].visited=false;
        }
        for (int i=12;i<18;i++)
        {
            ...
            checkers[i].cell=board[i-11][0];
            board[i-11][0].checker=checkers[i];
            checkers[i].alive=true;
            checkers[i].visited=false;
        }
        for (int i=18;i<24;i++)
        {
            ...
            checkers[i].color=Color.black;
            checkers[i].colorCode=1;
            checkers[i].cell=board[i-17][boardsize-1];
            board[i-17][boardsize-1].checker=checkers[i];
            checkers[i].alive=true;
            checkers[i].visited=false;
        }
